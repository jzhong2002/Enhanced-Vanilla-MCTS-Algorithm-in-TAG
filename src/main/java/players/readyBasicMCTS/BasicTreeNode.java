...
class BasicTreeNode {
    ...
    private AbstractAction selectHeuristicAction(AbstractGameState rolloutState) {
        List<AbstractAction> availableActions = player.getForwardModel().computeAvailableActions(rolloutState, player.parameters.actionSpace);
        ...
        // use epilson for heuristic rollouts, sometimes use random action for exploration
        ...
        for (AbstractAction action : availableActions) {
            ...
            player.getForwardModel().next(nextState, action.copy()); // Duplicate original gs for heuristic evaluation\

            // use heuristic evalutate for the copied game/resulting state based on a value
            // create a value variable to store heuristic value in the resulting state
            double value = player.getParameters().getStateHeuristic().evaluateState(nextState, player.getPlayerID());

            // add a small noise to break ties e.g. 0.0051 vs 0.00054
        ...
        return bestAction != null ? bestAction : availableActions.get(rnd.nextInt(availableActions.size()));
    }
     ...
     */
    // rollOut() - performs Monte Carlo simulation step
    private double rollOut() {
        ...
        if (player.getParameters().rolloutLength > 0) {
            while (!finishRollout(rolloutState, rolloutDepth)) {
                ...
                // choose rollout based on parameter policy
                if (player.getParameters().useHeuristicRollouts) {
                    ...
                    next = randomPlayer.getAction(rolloutState, randomPlayer.getForwardModel().computeAvailableActions(
